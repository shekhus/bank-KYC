...
contract KYC {
   ...
   // string[] verifiedCustomerNamesList;
    ...
    function addCustomer(string memory _userName, string memory _customerData) public returns (uint8) {
        require(customers[_userName].bank == address(0), "n  ");
        require(banks[msg.sender].bankRating > 50, "r ");
    ...
    function removeKYCRequest(string memory _userName, string memory _customerData)  public returns (uint8) {
        ...
        require(stringsEquals(kycRequests[_customerData].data_hash, _customerData) , " ");
        require(stringsEquals(kycRequests[_customerData].userName, _userName), "s");
    ...
    modifier isUsernameExists(string memory _userName) {
        require(stringsEquals(customers[_userName].userName, _userName), "t");
        _;
    }
