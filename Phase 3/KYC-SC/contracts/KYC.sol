...
contract KYC {
   ...
   // string[] verifiedCustomerNamesList;
    ...
    function addCustomer(string memory _userName, string memory _customerData) public returns (uint8) {
        require(customers[_userName].bank == address(0), "This customer is already added, please call modifyCustomer to edit the customer data");
        require(banks[msg.sender].bankRating > 50, "Low rating! Bank is not permitted to add the customer ");
    ...
    function removeKYCRequest(string memory _userName, string memory _customerData)  public returns (uint8) {
        ...
        require(stringsEquals(kycRequests[_customerData].data_hash, _customerData) , "The user data does not exist in kycRequests");
